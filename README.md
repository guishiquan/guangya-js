...
油猴脚本，在光鸭云盘网页右侧加一条窄工具条：勾选文件夹/文件 → 点下载图标 → 自动取签名直链 → 推到 aria2。**aria2 落地的目录结构与云盘保持一致**。

## 这个脚本能做什么

- 在 `https://guangyapan.com` 系列页面右侧贴边注入一条 44 px 宽的垂直工具条（带状态点 + 操作图标）
...
3. 访问 `https://guangyapan.com/`，浏览器右侧应该出现一条深色工具条
...
| 元素 | 含义 |
|---|---|
| 顶部小圆点 ×2 | 状态指示：上=认证，下=当前目录。绿色=就绪、红色=缺失。hover 出完整文字说明 |
| 顶部短横条 | 拖动手柄，按住可上下挪动工具条位置 |
| ⬇ 下载（蓝色高亮） | 推送当前勾选到 aria2（主操作） |
| 👁 眼睛 | 展开预览：跑「勾选 → 展开」但不推送，弹出 popover 列出每个文件的目标路径 |
| ⚡ 闪电 | 测试 aria2 连通（调 `aria2.getVersion`），结果写日志 |
| ⚙ 齿轮 | 打开 / 关闭设置 popover |
| ☰ 列表 | 打开 / 关闭日志 popover；日志标题栏右上角有 🗑 垃圾桶可一键清空当前日志 |
| › 折叠 | 把工具条收成 10 px 细条贴边；再点 ‹ 恢复 |

Popover 关闭方式：再点工具条同一图标 / 点 popover 右上角 ×× / 按 ESC。
...
| 调试日志 | 勾选后把每次拦到的 guangyapan 请求 URL、列表项数都打到日志，排错用 |

设置 popover 下方还有「手填光鸭认证」三个字段（`Bearer …` / `did` / `dt`）。**正常情况留空即可**——脚本会自己从你刷新过的 API 请求里拿。手填值只在自动捕获失败或你想覆盖时才需要。
...
2. 看工具条顶部两个状态点：
   - 上面那个绿了 = 认证已就绪（hover 文字：`已就绪：Bearer / did / dt`）
   - 下面那个绿了 = 当前目录已拦截（hover 文字：`✓拦截器已装 | parentId=… 已缓存 N 项`）
3. 勾选你想下载的若干文件夹/文件（可混合可多选）
4. 点 ⬇ 下载图标 —— 完整跑「展开 → 取直链 → 推送」
   - 或先点 👁 眼睛预览：会跑「展开」但不推送，弹出预览 popover 列出每个文件的目标落地路径
   - 或先点 ⚡ 闪电：调一次 `aria2.getVersion`，结果显示在日志里
5. 整个过程进度都写到日志。点 ☰ 列表图标打开日志 popover 查看，例如：
   ```
   勾选 1 项：测试A
   展开完成，共 12 个文件
   直链就绪 12/12
   ...
   ```
6. 想清空日志：点日志 popover 标题栏右上角的 🗑 垃圾桶按钮

## 排错

| 现象 | 怎么办 |
|---|---|
| 下面那个状态点是红的（拦截器未确认 / 当前目录未拦截）| 强刷新页面 `Ctrl+Shift+R`，确保脚本在 `document-start` 抢在页面前装好 hook |
| 上面那个状态点是红的（认证缺失） | 在云盘里进任一目录刷一次列表请求；或打开设置 popover，在「手填光鸭认证」里粘贴一份 |
| `[list] → 0 项` | 设置里勾选「调试日志」，下一次列表请求会带 `[shape]` 信息（顶层 keys、数组分布、首元素 keys）；贴回给开发者，把字段名加进 `ID_KEYS`/`NAME_KEYS` 即可 |
...
| 工具条挡住了云盘 UI | 点 › 折叠图标缩成细条；按住顶部小横手柄可上下挪动 |
...
| 路径 | 说明 |
|---|---|
| `光鸭云盘-aria2助手.user.js` | 主脚本（约 1350 行） |
